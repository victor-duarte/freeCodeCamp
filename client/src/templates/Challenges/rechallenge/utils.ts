...
import { Catch, RequiredItem } from './prop-types';
 ...
 * @returns {object} File
 */
export function getInitialHtmlFile(files: File[]): File | undefined {
 ...
 */
function wasHtmlFile(file: File): boolean {
 ...
 */
export function getSource(files: File[], indexHtml?: File | undefined): string {
  return files.reduce((source: string, file: File): string => {
